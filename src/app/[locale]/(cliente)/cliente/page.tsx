...
import { Heart, CalendarRange, ArrowRight, UserCheck, Shield } from "lucide-react";
...
export default function ClienteDashboard() {
  ...
  // Fetch count of active requests from LocalStorage
  useEffect(() => {
    ...
    <div className="flex flex-col gap-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        {stats.map((stat, idx) => (
          <Link key={idx} href={stat.link} className="block group">
            <div className="p-5 border border-[var(--border)] bg-[var(--surface)] hover:border-[var(--accent)] hover:shadow-md rounded-sm transition-all flex items-center justify-between">
              <div>
      ...
      <div className="p-5 border border-emerald-500/20 bg-emerald-500/5 rounded-sm flex items-start gap-4">
        ...
        <div className="text-xs leading-relaxed text-emerald-300">
          <h5 className="font-bold mb-1">{locale === "es" ? "Conexión Segura TLS" : "TLS Secure Connection"}</h5>
      ...
      <div className="flex flex-col gap-4">
        ...
        {isLoading ? (
          ...
          <div className="p-8 border border-[var(--border)] bg-[var(--surface)]/30 rounded-sm text-center text-xs text-[var(--text-muted)] font-mono">
            {locale === "es"
